...
class Filter:
    ...
    def __init__(self, config: Optional[FilterConfig] = None) -> None:
        ...
        self.word_boundaries = config.get("word_boundaries", not self.allow_obfuscated_match)
    ...
    def _evaluate_severity(self, word: str, text: str) -> Optional[SeverityLevel]:
        """Evaluate the severity level of a match."""
        regex = self._get_regex(word)
        
        if regex.search(text):
            return SeverityLevel.EXACT
        elif self._is_fuzzy_tolerance_match(word, text):
    ...
    def is_profane(self, value: str) -> bool:
        ...
        """
        input_text = self._normalize_obfuscated(value) if self.allow_obfuscated_match else value
    ...
    def check_profanity(self, text: str) -> CheckProfanityResult:
        ...
        """
        input_text = self._normalize_obfuscated(text) if self.allow_obfuscated_match else text
        ...
        result["reason"] = (
            ...
            if matches else "No profanity detected"
        )
    ...
    def check_profanity_with_min_severity(
        ...
        if result.get("severity_map") and result.get("profane_words"):
            filtered_words = [
                word for word in result["profane_words"]
                if result["severity_map"].get(word, 0) >= min_severity  # type: ignore
            ]
